...
function crawler_coles(element) {
  for (let i = 1; i < element.p; i++) {
    setTimeout(async () => {
      ...
      await page.goto(
        `https://shop.coles.com.au/a/a-vic-metro-richmond-south/everything/browse/${element.m}/${element.c}?pageNumber=${i}`
      );
...
new CronJob(
  ...
  function() {
    const element = [
      ...
      '/665/skin-care',
      '/648/cosmetics',
      '/159/oral-hygiene-and-dental-care',
      '/792/household',
      '/129/hair-care',
      '/1255/sports-nutrition'
    ];
        ...
        request(
          ...
          function(error, res, body) {
            if (error) {
              ...
              const item = $('.pager-count')
                .text()
                .replace('  Results ', '');

              const paging = parseInt(parseInt(item) / 24) + 2;
        ...
        if (data != 'no results') {
          ...
          for (let i = 0; i < data.length; i++) {
            if (data[i].discount != 0) {
              content +=
                `<a href='${data[i].link}'><img style="height:150px;width:150px;" src='${data[i].img}'></a><br>` +
                `<span>${data[i].title} at ${data[i].store} now is $${data[i].price}</span>` +
                `<span style="color:red;"> save $${data[i].discount}</span><br><br>`;
            }
          }
          ...
          mailTransport.sendMail(
            {
              ...
              html: `<h1>Your tacking list for this week:</h1>
			    <h3>${content}</h3>`
            },
            function(err) {
              if (err) {
