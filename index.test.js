...
test('wishlist add test', () => {
  return dao.product
    .wishlist('10103', 'bigw', '3d42ef457876b74874be115c228db6fc51f71708e897f0093669b18a2d0d1642')
    .then(data => {
  ...
  request(
    ...
    function(error, res, body) {
      if (error) {
        ...
        let price = $('.product-container')
          .find('.Price')
          .text()
          .replace(/\s\s+/g, '')
          .replace('$', '');
        let discount = $('.product-container')
          .find('.Price')
          .next()
          .text()
          .replace(/\s\s+/g, '')
    ...
    function(error, res, body) {
      if (error) {
        ...
        let link =
          ...
          id +
          '/' +
          body.Bundles[0].Products[0].UrlFriendlyName;
        let subTitle =
          body.Bundles[0].Products[0].UrlFriendlyName +
          '-' +
          body.Bundles[0].Products[0].PackageSize;
